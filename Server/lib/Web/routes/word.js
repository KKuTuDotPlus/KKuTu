...
Server.get("/word", function(req, res){
	if(!checkWord(req, res)) return;
	
	req.session.word = true;
	page(req, res, "word");

});

Server.get("/word/injeong", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.get("/word/gamsi", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.get("/word/users", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.get("/word/kkutudb/:word", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.get("/word/kkututheme", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.get("/word/kkutuhot", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.get("/word/shop/:key", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.post("/word/injeong", function(req, res){
	if(!checkWord(req, res)) return;
...
Server.post("/word/kkutudb", onKKuTuDB);
...
Server.post("/word/kkutudb/:word", function(req, res){
	if(!checkWord(req, res)) return;
	if(req.body.pw != GLOBAL.PASS) return res.sendStatus(400);
	var TABLE = MainDB.kkutu[req.body.lang];
...
Server.post("/word/kkutuhot", function(req, res){
	if(!checkWord(req, res)) return;
	if(req.body.pw != GLOBAL.PASS) return res.sendStatus(400);
...
Server.post("/word/users", function(req, res){
	if(!checkWord(req, res)) return;
	if(req.body.pw != GLOBAL.PASS) return res.sendStatus(400);
...
Server.post("/word/shop", function(req, res){
	if(!checkWord(req, res)) return;
	if(req.body.pw != GLOBAL.PASS) return res.sendStatus(400);
...
function checkWord(req, res){
	if(global.isPublic){
		if(req.session.profile){
			if(GLOBAL.WORD.indexOf(req.session.profile.id) == -1){
				req.session.word = false;
